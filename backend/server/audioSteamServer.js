...
io.on('connection', function (socket) {
    ...
    socket.on('audioStream', async function (audioData) {
        ...
        const audioBase64 = audioData.base64; // Base64 데이터를 받음
        if (audioBase64) {
            try {
...
const processAudioData = async (audioFilePath) => {
    try {
         const response = await axios.post('http://192.168.0.48:5000/analyze_audio', {
...
const clearAudioFolder = () => {
    const audioFolderPath = path.join('./audio_data', 'audioFiles');

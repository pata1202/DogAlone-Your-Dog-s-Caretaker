...
export default function LGwaterCard() {
  const [lastFeedTime, setLastFeedTime] = useState(new Date()); // 마지막 급수 시간
  const [nextFeedTime, setNextFeedTime] = useState(new Date()); // 예정 급수 시간
  ...
  return (
    <View style={styles.cardContainer}>
      {/* LG 급수 텍스트 */}
      <Text style={styles.titleText}>LG 급수기</Text>

      {/* 펫케어 이미지 */}
      <Image source={require('../dogAloneAssets/water.png')} style={styles.waterImage} />
      ...
      {/* 마지막 배식 시간 버튼 */}
      <Text style={styles.labelText2}>마지막 급수 시간</Text>
      <TouchableOpacity
      ...
      {/* 예정된 배식 시간 버튼 */}
      <Text style={styles.labelText}>예정된 급수 시간</Text>
      <TouchableOpacity
